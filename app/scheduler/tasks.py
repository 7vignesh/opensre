...
is unavailable or returns no findings, a clear "no data" message is
delivered so operators know the system ran but found nothing.
"""
...
def _build_daily_summary(task: ScheduledTask) -> str:
    ...
    window. Returns the pipeline report if available, otherwise a clear
    'no incidents' message indicating the system ran successfully.
    """
    ...
    try:
        ...
        logger.warning("Daily summary pipeline query failed for task %s: %s", task.id, exc)
        # Fall through to the no-data message rather than crashing —
        # a daily summary with "no data available" is better than no delivery.
...
def _build_weekly_audit(task: ScheduledTask) -> str:
    ...
    window. Returns the pipeline report if available, otherwise a clear
    'no alerts' message indicating the system ran successfully.
    """
    now = datetime.now(UTC)
    window_start = now - timedelta(hours=task.window_hours)
    ...
    try:
        ...
        logger.warning("Weekly audit pipeline query failed for task %s: %s", task.id, exc)
    ...
    return (
        ...
        f"No alert data available from configured integrations.\n\n"
        f"<i>Generated by OpenSRE scheduled delivery</i>"
    )
